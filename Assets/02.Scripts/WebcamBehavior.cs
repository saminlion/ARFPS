...
public class WebcamBehavior : MonoBehaviour
{
    ...
    public int m_framesPerSecond = 0;
    ...
    void OnGUI()
    {
        ...
        if (Application.HasUserAuthorization(UserAuthorization.WebCam))
        {
            ...
            else
            {
                ...
                for (int index = 0; index < WebCamTexture.devices.Length; ++index)
                {
                    ...
                    if (GUILayout.Button(string.Format("web cam device {0}{1}{2}", m_indexDevice == index ? "[" : string.Empty, device.name, m_indexDevice == index ? "[" : string.Empty), GUILayout.MinWidth(200), GUILayout.MinHeight(50)))
                    {
                        m_indexDevice = index;
        ...
        {
            GUILayout.Label("Pending WebCam Authroization..");
        }
    }
    ...
    void Update()
    {
        if (null != m_texture && m_texture.didUpdateThisFrame)
        {
            CameraMaterial.mainTexture = m_texture;
        }
    }
